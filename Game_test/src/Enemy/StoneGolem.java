...
public class StoneGolem extends Entity {
    ...
    public void do_action() {
        ...
        if (dying_animation == 5 * dying_animation_delay) {
            for(int i=x/gp.tileSize-2; i< x/gp.tileSize+width/gp.tileSize+1; i++ )
            {
                for(int j=y/gp.tileSize-3; j<y/gp.tileSize+height/gp.tileSize-1; j++)
                    gp.tileM.mapTileNum[gp.currentMap][i][j]=25;
            }

            gp.tileM.mapTileNum[gp.currentMap][x/gp.tileSize+width/gp.tileSize/2][y/gp.tileSize+height/gp.tileSize-2]=24;
